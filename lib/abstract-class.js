...
AbstractClass.hasAndBelongsToMany = function (anotherClass, params) {
    ...
    */
    ...
    var clsId = railway.utils.camelize( this.modelName + '_id' ),
    plur = railway.utils.pluralize( anotherClass.modelName ),
    manys = plur.toLowerCase();

    var proxied = this.prototype.populate;
    this.prototype.populate = function(cb) {
        var self = this;
        var where = {};
        where[clsId] = this.id;
        anotherClass.all({join:params.through, where:where}, function(err, objs) {
            self[manys] = objs;
