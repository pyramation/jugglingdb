...
function populate(Class, Many, Through) {
    ...
    var clsId = railway.utils.camelize( Class.modelName + '_id' ),
    plur = railway.utils.pluralize( Many.modelName ),
    ...
    Class.prototype.populate = function(cb) {
        ...
        var where = {}; 
        where[clsId] = this.id;
        Many.all({join:Through, where:where}, function(err, objs) {
            self[manys] = objs;
...
AbstractClass.hasAndBelongsToMany = function (anotherClass, params) {
    var relation = params.through.modelName,
    utils = railway.utils,
    accessorName1 = utils.camelize(anotherClass.modelName),
    accessorName1s = utils.camelize(utils.pluralize(anotherClass.modelName)),
    fk1 = utils.camelize(anotherClass.modelName + '_id'),
    accessorName2 = utils.camelize(this.modelName),
    accessorName2s = utils.camelize(utils.pluralize(this.modelName)),
    fk2 = utils.camelize(this.modelName + '_id');

    params.through.belongsTo(this.modelName, {as: accessorName1, foreignKey: fk1});
    params.through.belongsTo(anotherClass.modelName, {as: accessorName2, foreignKey: fk2});
