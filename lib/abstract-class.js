...
AbstractClass.prototype._initProperties = function (data, applySetters) {
    ...
    if (settings.aliased  && settings.aliased.length) {
        settings.aliased.forEach( function(alias) {
...
AbstractClass.findOne = function findOne(params, cb) {
    ...
    this.all(params, function (err, collection) {
        ...
        if (!collection || !collection.length > 0) return cb(err, null);
        cb(err, collection[0]);
    });
};
...
AbstractClass.prototype.toJSON = function () {
    ...
    var ds = ctor.schema.definitions[ctor.modelName];
    var settings = ds.settings;
    var obj = _.extend({}, this.toObject());

    var keys = Object.keys(obj);
    var args = [keys];
    if (settings.hidden && settings.hidden.length) {
        [].push.apply(args, settings.hidden);
    }
    var keep = _.without.apply({},  args);
    var cleanObj = {};
...
var utils  = require('../../../helpers/inflection.js');
var pluralize = utils.pluralize;
var casingFunc = utils.underscore; // camelize;
function populate(Class, Many, Through) {

  var clsId = casingFunc(Class.modelName + '_id'),
    plur = pluralize(Many.modelName),
    manys = plur.toLowerCase();

  var proxied = Class.prototype.populate;
  Class.prototype.populate = function(cb) {
    var self = this;
    var where = {};
    where[clsId] = this.id;
    Many.all({
      ...
      self[manys] = objs;
      if (typeof proxied == "function") proxied.call(this, cb);
...
AbstractClass.hasAndBelongsToMany = function(anotherClass, params) {

    var through = params.through,
    accessorLeft = pluralize(anotherClass.modelName),
    accessorRight = pluralize(this.modelName),
    fkLeft = casingFunc( params.foreignKey || (anotherClass.modelName + '_id')),
    fkRight = casingFunc( this.modelName + '_id' );

    through.belongsTo(this.modelName, {as: accessorLeft, foreignKey: fkRight});
    through.belongsTo(anotherClass.modelName, {as: accessorRight, foreignKey: fkLeft});

  /**
   * create the populate function
   */

  populate(this, anotherClass, params.through);
  populate(anotherClass, this, params.through);
};


// https://github.com/derAndreas/jugglingdb/tree/hasAndBelongsToMany
AbstractClass.hasAndBelongsToManyDerAndreas = function(anotherClass, params) {
    ...
    this.prototype['__cachedRelations__'] = this.prototype['__cachedRelations__'] || {};
    this.prototype['__cachedRelations__'][accessorLeft] = []
    ...
    defineScope(this.prototype, anotherClass, methodName, function () {
        var x = {};
        x[fkRight] = this.id;
        return {where: x, join: {modelName: through.modelName, foreignKey: fkLeft}};
    }, {
        add: add,
        save: save
    });
    ...
    if(inverse) {
        anotherClass.hasAndBelongsToMany(this, {as: inverse.as, through: through});
    }

    function add(p, cb) {

        if(!cb) {
            throw new Error('No Callback functions provided for hasAndBelongsToMany add function');
        }

        if(p instanceof AbstractClass) {
            this['__cachedRelations__'][accessorLeft].push(p);
            cb(null)
        } else if(Array.isArray(p)) {
            for(var i = 0, len = p.length; i < len; i++) {
                this['__cachedRelations__'][accessorLeft].push(p[i]);
            }
            cb(null)
        } else if((p === +p) || (p.toString() === '[object Number]')) {
            anotherClass.find(p, function (err, inst) {
                if (err) return cb(err);
                if (inst[fkLeft] == p) {
                    this['__cachedRelations__'][accessorLeft].push(inst);
                    cb(null);
                } else {
                    cb(new Error('Permission denied'));
                }
            ...
            throw new Error('Unknown type to add through hasAndBelongsToMany()');
        }
    };

    function save(cb) {
        var data = this['__cachedRelations__'][accessorLeft],
            waitForDone = 0;

        if(!this.id) {
            return cb('Cannot save accessociated models, source model "%s" is not saved', this.modelName)
        }
        ...
        data.forEach(function(el) {
            ...
            if(el.id) {
                ...
                through.create(x, function(err, throughResult) {
                    saveDone(err, el, throughResult);
                });
            } else {
        ...
        function saveDone(err, association, through) {
            result.push(association);
            resultThrough.push(through);

            if(--waitForDone === 0) {
                cb(null, result, resultThrough);
            }
        }
    };
