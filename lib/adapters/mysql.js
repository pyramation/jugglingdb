...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter) {
        ...
        if (filter.join) {
            ...
            if (typeof filter.join === 'object') {
                ...
                Image.*, File.url, ImageAssoc.weight
                FROM Image
                INNER JOIN Image ON Image.file_id = File.id INNER JOIN ImageAssoc ON ImageAssoc.image_id = Image.id WHERE ImageAssoc.`image_gallery_id` = NULL
                ...
                */
                ...
                _.each(filter.join, function(info, Model) {
                    if (info.inverse) {
                        joins.push(buildInverseJoin(thisTable, Model)); 
                    } else {
                        joins.push(buildJoin(thisTable, Model));
                    }
                });
                sql += ' ' + joins.join(' ');
                ...
                if (filter.where) {
                    wheres.push(filter.where, thisTable)
                }
    ...
    function buildWhere(conds, Table) {
        ...
        Object.keys(conds).forEach(function (key) {
            ...
            var val = null;
            if (Table) {
                val = self.toDatabase(self._models[Table].properties[key], conds[key]);
            } else {
    ...
    function buildJoin(thisClass, relationClass) {
        ...
        qry = thisClass,
        f1 = rel + '.' +  casingFunc( qry + '_id'),
        f2 = qry + '.id';
        return 'INNER JOIN ' + rel + ' ON ' + f1 + ' = ' + f2;
    }

    function buildInverseJoin(thisClass, relationClass) {
        var rel = relationClass,
        qry = thisClass,
        f1 = qry + '.' +  casingFunc( rel + '_id'),
        f2 = rel + '.id';
    ...
    function buildOrderBy(order) {
        ...
        return 'ORDER BY ' + order.join(', ') + ' desc'
    }
